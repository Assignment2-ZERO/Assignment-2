...
void Write_User(string user_no, string user_name, string mssv, string birth, string job, string email);//Viết thông tin người dùng vào file
bool Check_Infomation_User(int what_check, string str);//Kiểm tra thông tin người dùng có tồn tại trong file hay chưa
...
void Menu_Libpro()
{
	...
	do
	{
		...
		Text_Menu("2. Dang ki nguoi dung.");
...
void SignIn_User()
{
	...
	do
	{
		...
		user_no = user.getuser_no();// Nhập vào số thự tự 
		Check = true;
		...
		else//User_no ton tai trong data. 
		{
			...
			if (Check == true)//Tên user trong file không giống với tên user đã nhập
			{
				Sign_User_Fail(SignIn, " So thu tu nhap dung nhung ten nguoi dung khong dung !!!", "Dang nhap");
				break;
			}
			else//Tên user trong file  giống với tên user đã nhập
			{
...
void SignUp_User()
{
	...
	do
	{
		...
		if (user_name.empty())//Nếu không nhập gì thì đăng kí lại
		{
		...
		getline(cin, Mssv);
		Check = CheckInt(Mssv);
		if (Check == false)//Kiểm tra mã số sinh viên có phải là chuỗi số không
		{
			Sign_User_Fail(SignUp, "Ma so sinh vien nhap sai !!!", "Dang ki");
			break;
		}
		Check = Check_Infomation_User(MSSV, Mssv);
		if (Check == false)//Kiểm tra thông tin người dùng có trong file không
		{
		...
		//Kiểm tra email đã tổn tại trong data chưa
		if (Check == false)
		{
